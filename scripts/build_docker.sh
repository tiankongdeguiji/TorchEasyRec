...
do
    docker build --network host -t ${REGISTRY}/${REPO_NAME}:${DOCKER_TAG}-${DEVICE}${DOCKER_TAG_SUFFIX} --build-arg DEVICE=${DEVICE} --build-arg CACHE_BUST_REQ=u3 ${PIP_MIRROR:+--build-arg PIP_MIRROR=${PIP_MIRROR}} .
